...
export async function importSwagger(request, env, { params }) {
  const db = new DatabaseAdapter(env.DB);
  const body = await parseBody(request);
  console.log(`[Swagger Import] Project ${params.id}:`, body ? `Fetching from ${JSON.stringify(body)}` : 'Using provided spec');
...
export async function generateTests(request, env, { params }) {
  ...
  for (const endpoint of endpointsList.slice(0, limit)) {
    try {
      ...
      const cases = await generateTestCases(env.AI, ep, fullSpec, r.db, params.id, env.GEMINI_API_KEY);
...
export async function runExecutionInline(executionId, projectId, env, r) {
  ...
  for (const result of results) {
    ...
    if (result.status === 'failed' || result.status === 'error') {
      ...
      if (tc && ep) {
        const bug = await analyzeBug(env.AI, { endpoint: ep, testCase: tc, result }, r.db, projectId, env.GEMINI_API_KEY);
        if (bug) {
...
export async function detectApiWorkflows(request, env, { params }) {
  ...
  const workflows = await detectWorkflows(env.AI, endpoints, epRepo.db, params.id, env.GEMINI_API_KEY);
...
async function analyzeFlowRunBugs(db, env, suite, allSteps, results, runId) {
  ...
  for (const result of failedResults) {
    try {
      ...
      const bug = await analyzeFlowStepBug(env.AI, { step, result, endpoint, suiteContext }, db, suite.project_id, env.GEMINI_API_KEY);
      if (!bug) continue;
...
[
  ...
  try {
    ...
    const { steps, droppedInvalid, droppedDuplicates } = await runAILogged(
      ...
      { stage: 'flow_step_generation', projectId: project?.id || null, geminiKey: env.GEMINI_API_KEY }
    );
