    ...
    return (
        <div style={{ minHeight: '100vh', background: 'var(--bg-app)' }}>
            ...
            <div style={{ maxWidth: 760, margin: '0 auto', padding: isPublic ? '48px 24px' : '28px 32px' }}>
                ...
                {/* Data risk summary table */}
                <div style={{ marginBottom: 40 }}>
                    <h2 style={{ fontSize: 16, fontWeight: 700, marginBottom: 4 }}>At a Glance — Data Risk Summary</h2>
                    <p style={{ fontSize: 12, color: 'var(--text-tertiary)', marginBottom: 14 }}>What we store, where it goes, how long we keep it, and what risks to be aware of.</p>
                                ...
                                {[
                                    {
                                        ...
                                        dest: 'Cloudflare D1 Database',
                                        ret: 'Lifetime of account\n(deleted within 30 days of request)',
                                        risk: 'low',
                                        note: 'Low. Standard SaaS practice. Passwords are hashed, not stored.',
                                    },
                                    {
                                        type: 'API Schemas\n(Swagger/OpenAPI specs)',
                                        dest: 'Cloudflare D1 + Workers AI (inference only)',
                                        ret: 'Indefinite while account active',
                                        risk: 'low',
                                        note: 'Low. Structural data only — paths, methods, schemas. No live user payload data.',
                                    },
                                    {
                                        type: 'Auth Credentials\n(Passwords/tokens for your API)',
                                        dest: 'Cloudflare D1 Database\n(AES-256-GCM encrypted)',
                                        ret: 'Indefinite until you delete the flow suite or run',
                                        risk: 'medium',
                                        note: 'Medium. Stored to execute tests. Encrypted at rest. Mitigation: Use dedicated test accounts only — never production credentials.',
                                    },
                                    {
                                        type: 'API Responses\n(Data your endpoints return)',
                                        dest: 'Cloudflare D1 Database\n(plain text)',
                                        ret: 'Indefinite until you delete the run',
                                        risk: 'high',
                                        note: 'High. Can contain live PII if you test production endpoints. Mitigation: Only test against mock or staging environments with anonymised data. Delete runs when done.',
                                    },
                                    {
                                        type: 'Bug Analysis Excerpts\n(Truncated failed step data)',
                                        dest: 'Cloudflare Workers AI\n(inference, not stored)',
                                        ret: 'Subject to Cloudflare\'s infrastructure limits — not stored by HitAPI',
                                        risk: 'medium',
                                        note: 'Medium. Truncated to 200 chars per request/response. Mitigation: Ensure error messages from your API do not echo back secrets or sensitive values.',
                                    },
                                    {
                                        type: 'Terms Acceptance Records\n(IP, user-agent, timestamp)',
                                        dest: 'Cloudflare D1 Database',
                                        ret: 'Account lifetime + 7 years\n(legal compliance requirement)',
                                        risk: 'low',
                                        note: 'Low. Required for legal proof of consent. Contains IP address and browser user-agent only.',
                                    },
                                ].map((row, i) => {
                                    const riskColor = row.risk === 'high' ? 'var(--red)' : row.risk === 'medium' ? 'var(--amber)' : 'var(--green)';
                                    const riskBg = row.risk === 'high' ? 'rgba(255,92,92,0.08)' : row.risk === 'medium' ? 'rgba(255,181,71,0.08)' : 'rgba(35,209,139,0.08)';
                                    return (
                                        <tr key={i} style={{ borderBottom: '1px solid var(--border)', verticalAlign: 'top' }}>
                                            <td style={{ padding: '10px 12px', color: 'var(--text-primary)', fontWeight: 500, whiteSpace: 'pre-line', fontSize: 12 }}>{row.type}</td>
                                            <td style={{ padding: '10px 12px', color: 'var(--text-secondary)', whiteSpace: 'pre-line', fontSize: 11 }}>{row.dest}</td>
                                            <td style={{ padding: '10px 12px', color: 'var(--text-secondary)', whiteSpace: 'pre-line', fontSize: 11 }}>{row.ret}</td>
                                            <td style={{ padding: '10px 12px', fontSize: 11 }}>
                                                <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 4, fontWeight: 700, fontSize: 10, background: riskBg, color: riskColor, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.04em' }}>
                                                    {row.risk}
                                                </span>
                                                <div style={{ color: 'var(--text-secondary)', lineHeight: 1.6 }}>{row.note}</div>
                                            </td>
                                        </tr>
                                    );
                                })}
